...
export default function LockUserDialog({
  ...
  const actionTitle = isActive ? 'Khóa tài khoản' : 'Mở khóa tài khoản';
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast({
        title: 'Thành công!',
        description: `Đã ${action} tài khoản ${user.username} thành công`,
        variant: 'default'
      });
      onSuccess();
      onClose();
              ...
              {isActive ? (
                <div className="rounded-lg border border-red-200 bg-red-50 p-3">
                  <p className="text-sm text-red-800">
                    <strong>Lưu ý:</strong> Sau khi khóa, người dùng sẽ không
                    thể đăng nhập vào hệ thống. Tất cả phiên đăng nhập hiện tại
                    sẽ bị hủy.
                  </p>
                </div>
              ) : (
                <div className="rounded-lg border border-green-200 bg-green-50 p-3">
                  <p className="text-sm text-green-800">
                    <strong>Lưu ý:</strong> Sau khi mở khóa, người dùng sẽ có
